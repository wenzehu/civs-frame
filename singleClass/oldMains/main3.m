    ...
    for iSample= 1:numSample
        ...
        for cx = 1:size(img,1)
            for cy = 1:size(img,2)
                v0 = currSample(cx,cy);
		iVal = 1:8;
		pState = lambdaI(cx,cy)*(iVal-v0) + energy;
        pState = pState-min(pState);
		expState = exp(pState);
                ...
                v1 = v1(1);
                energy = energy + lambdaI(cx,cy)*(v1-v0);
                currSample(cx,cy)=v1;
            end
        end
