function em_mixtureFRAMEFuncV3(trainImges,modelPath,cachePath)
	...
	for iFilter = 1:numFilter
		...
		gpuSUM1mapFind{iEl}=gpuArray(SUM1mapFind{iEl});
            ...
            for (flip = 0 : flipOrNot)
                for (rot = -rotateShiftLimit : rotateShiftLimit)
                    r = rot+rotateShiftLimit+1 + (rotateShiftLimit*2+1)*flip;
                    [allFx, allFy,MAX2score] = FRAME_SUM2_LogZV3(numResolution, allSizex, allSizey, numFilter, alllambda(r, :), logZ, ImageMultiResolution ,gpuSUM1mapFind,halfTemplatex,halfTemplatey,halfFilterSizes(1));
